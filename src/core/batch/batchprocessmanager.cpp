...
void BatchProcessManager::setState(BatchProcessState newState) {
    if (m_state != newState) {
        ...
        QString stateStr;
        switch (newState) {
            case BatchProcessState::Idle: stateStr = "空闲"; break;
            case BatchProcessState::Running: stateStr = "运行中"; break;
            case BatchProcessState::Paused: stateStr = "已暂停"; break;
            case BatchProcessState::Completed: stateStr = "已完成"; break;
            case BatchProcessState::Cancelled: stateStr = "已取消"; break;
        }
...
void BatchProcessManager::onAIAnalysisComplete(const AIAnalysisResponse& response) {
    ...
    if (response.success) {
        ...
        if (retryCount <= m_config.maxRetryCount) {
            ...
            m_processQueue.enqueue(func);
            Logger::instance().warning(QString("函数分析失败，将重试 (%1/%2): %3")
                .arg(retryCount).arg(m_config.maxRetryCount).arg(func.name));
        } else {
            m_failedCount++;
...
void BatchProcessManager::processNext() {
    ...
    Logger::instance().info(QString("开始分析函数 (%1/%2): %3")
        .arg(m_currentIndex + 1).arg(m_totalCount).arg(func.name));
...
void BatchProcessManager::completeBatch() {
    ...
    Logger::instance().info(QString("批量处理完成: 成功 %1, 失败 %2, 跳过 %3")
        .arg(m_successCount).arg(m_failedCount).arg(m_skippedCount));
...
void BatchProcessManager::loadProcessState() {
    ...
    Logger::instance().info(QString("已加载处理状态: 已处理 %1, 待重试 %2")
        .arg(m_processedFunctions.size()).arg(m_failedFunctions.size()));
}
