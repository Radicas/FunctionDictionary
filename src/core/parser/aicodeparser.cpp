...
AICodeParser::AICodeParser(QObject *parent)
    : QObject(parent), m_networkManager(nullptr), m_currentReply(nullptr),
      m_timeoutTimer(nullptr), m_isParsing(false),
      m_timeoutMs(300000), // 5分钟超时时间
      m_receivedTokens(0) {
...
AICodeParser::~AICodeParser() { cancelParsing(); }
...
void AICodeParser::parseFile(const QString &filePath) {
  ...
  Logger::instance().info(QString("检测到语言类型: %1, 文件大小: %2 字节")
                              .arg(language)
                              .arg(code.size()));
...
void AICodeParser::parseCode(const QString &code, const QString &language,
  ...
  Logger::instance().info(
      QString("构建Prompt完成, 长度: %1 字符").arg(prompt.size()));
...
bool AICodeParser::isParsing() const { return m_isParsing; }
...
void AICodeParser::setTimeout(int timeoutMs) { m_timeoutMs = timeoutMs; }
...
void AICodeParser::onReplyFinished() {
  ...
  if (reply->error() != QNetworkReply::NoError) {
    QString error = "网络请求失败: " + reply->errorString() +
                    " (错误码: " + QString::number(reply->error()) + ")";
  ...
  Logger::instance().info(
      QString("AI响应内容长度: %1 字符").arg(aiResponse.size()));
  ...
  if (result.success) {
    ...
    Logger::instance().info(QString("AI代码解析完成，提取到 %1 个函数")
...
void AICodeParser::onNetworkError(QNetworkReply::NetworkError error) {
  Logger::instance().error(
      QString("网络错误: %1 (错误码: %2)")
          .arg(m_currentReply ? m_currentReply->errorString() : "未知")
          .arg(error));
}
    ...
    if (!choicesArray.isEmpty()) {
      ...
      if (choiceObj.contains("delta")) {
        ...
        if (deltaObj.contains("content")) {
          ...
          m_streamContent += content;
          m_receivedTokens++;

          if (m_receivedTokens % 10 == 0) {
            emit parseProgress("AI处理中",
                               QString("正在分析代码... 已接收 %1 个token")
                                   .arg(m_receivedTokens));
          }
        }
      }
    }
...
void AICodeParser::onTimeout() {
  ...
  emit parseFailed(QString("请求超时 (%1 秒)，请检查网络连接或增加超时时间")
...
QString AICodeParser::buildParsePrompt(const QString &code,
  ...
  QString prompt =
      QString(
          ...
          "      使用时需要注意的问题\",\n"
          "    \"flowchart\": \"使用mermaid "
          "flowchart语法绘制的函数内部执行流程图，清晰展示所有分支和循环\",\n"
          "    \"sequence_diagram\": \"使用mermaid "
          "sequenceDiagram语法绘制的函数调用时序图，展示函数与其他模块的交互（"
          "如无外部调用可为空字符串）\",\n"
          "    \"structure_diagram\": \"使用mermaid "
          "graph语法绘制的模块依赖关系图，展示该函数在系统中的位置和依赖关系\""
          "\n"
          "  }\n"
          "]\n\n"
          "重要要求：\n"
...
AIParseResult AICodeParser::parseAIResponse(const QString& aiResponse, const QString& filePath, const QString& language) const {
  ...
  if (error.error != QJsonParseError::NoError) {
    result.errorMessage =
        "解析JSON失败: " + error.errorString() +
        " (位置: " + QString::number(error.offset) + ")";
    Logger::instance().error(result.errorMessage);
    Logger::instance().error("JSON内容前1000字符: " + jsonStr.left(1000));
    Logger::instance().error("JSON内容后500字符: " + jsonStr.right(500));
    
    int openBraces = jsonStr.count('{');
    int closeBraces = jsonStr.count('}');
    int openBrackets = jsonStr.count('[');
    int closeBrackets = jsonStr.count(']');
    
    Logger::instance().error(QString("括号统计: { %1 个, } %2 个, [ %3 个, ] %4 个")
                               .arg(openBraces).arg(closeBraces)
                               .arg(openBrackets).arg(closeBrackets));
    
    if (openBraces > closeBraces) {
      int missing = openBraces - closeBraces;
      jsonStr += QString("}").repeated(missing);
      Logger::instance().warning(QString("尝试补充 %1 个缺失的 }").arg(missing));
    }
    ...
    jsonDoc = QJsonDocument::fromJson(jsonStr.toUtf8(), &error);
    if (error.error != QJsonParseError::NoError) {
      result.errorMessage = "JSON修复失败: " + error.errorString();
      Logger::instance().error(result.errorMessage);
      return result;
    }
  }
...
FunctionData
AICodeParser::parseFunctionFromJson(const QJsonObject &jsonObj,
                                    const QString &filePath,
  ...
  if (jsonObj.contains("parameters") && jsonObj["parameters"].isArray()) {
    ...
    funcData.parameters =
        QJsonDocument(paramsJsonArray).toJson(QJsonDocument::Compact);
  }
...
bool AICodeParser::readFileContent(const QString &filePath,
                                   QString &content) const {
  QFile file(filePath);
  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
    Logger::instance().error("无法打开文件: " + filePath +
                             ", 错误: " + file.errorString());
    return false;
  }
