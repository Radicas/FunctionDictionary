    ...
    , m_enabled(true)
{
    ...
    connect(this, &MessageBus::messagePublished, 
...
QList<int> MessageBus::subscribeMultiple(const QList<MessageType>& types, QObject* receiver, 
...
void MessageBus::publish(MessageType type, const QVariant& data, 
                          const QString& sender, MessagePriority priority)
{
    if (!m_enabled) {
        return;
    }
    ...
    Logger::instance().debug(QString("发布消息(同步) - 类型: %1, 发送者: %2")
                             .arg(static_cast<int>(type))
                             .arg(sender));
...
void MessageBus::publishAsync(MessageType type, const QVariant& data, 
                               const QString& sender, MessagePriority priority)
{
    if (!m_enabled) {
        return;
    ...
    Logger::instance().debug(QString("发布消息(异步) - 类型: %1, 发送者: %2")
                             .arg(static_cast<int>(type))
                             .arg(sender));
...
Message MessageBus::sendRequest(MessageType type, const QVariant& data, 
                                 const QString& sender, int timeout)
{
    ...
    QString correlationId = QString("req_%1_%2")
                            .arg(QDateTime::currentMSecsSinceEpoch())
                            .arg(QRandomGenerator::global()->bounded(1000000));
    request.correlationId = correlationId;
    
    int subscriptionId = subscribe(type, this, [&](const Message& msg) {
    ...
    if (!responseReceived) {
        Logger::instance().warning(QString("请求超时 - 类型: %1, 关联ID: %2")
                                   .arg(static_cast<int>(type))
                                   .arg(correlationId));
    }
...
void MessageBus::dispatchMessage(const Message& message)
{
    ...
    if (!m_subscribers.contains(message.type)) {
        Logger::instance().debug(QString("消息无订阅者 - 类型: %1")
    ...
    for (const SubscriberInfo& info : validSubscribers) {
        try {
            ...
            Logger::instance().error(QString("消息处理异常 - 类型: %1, 错误: %2")
                                     .arg(static_cast<int>(message.type))
                                     .arg(e.what()));
        } catch (...) {
            Logger::instance().error(QString("消息处理未知异常 - 类型: %1")
                                     .arg(static_cast<int>(message.type)));
        }
    }
}
