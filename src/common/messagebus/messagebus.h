...
public:
     ...
     * @return 订阅ID列表
     */
    QList<int> subscribeMultiple(const QList<MessageType>& types, QObject* receiver, 
                                  MessageHandler handler, int priority = 0);
     ...
     */
    void publish(MessageType type, const QVariant& data = QVariant(), 
                 const QString& sender = QString(), MessagePriority priority = MessagePriority::Normal);
     ...
     * @param priority 消息优先级
     */
    void publishAsync(MessageType type, const QVariant& data = QVariant(), 
                      const QString& sender = QString(), MessagePriority priority = MessagePriority::Normal);
     ...
     * @return 响应消息
     */
    Message sendRequest(MessageType type, const QVariant& data = QVariant(), 
                        const QString& sender = QString(), int timeout = 5000);
