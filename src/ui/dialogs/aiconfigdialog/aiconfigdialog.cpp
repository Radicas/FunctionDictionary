      ...
      m_isLoading(false)
{
    ...
    connect(&ModelListFetcher::instance(), &ModelListFetcher::modelsFetched, this, &AIConfigDialog::onModelsFetched);
    connect(&ModelListFetcher::instance(), &ModelListFetcher::fetchFailed, this, &AIConfigDialog::onFetchFailed);
...
void AIConfigDialog::setupUI()
{
    ...
    connect(m_configListWidget, &QListWidget::currentRowChanged, this, &AIConfigDialog::onConfigSelectionChanged);
...
void AIConfigDialog::onDeleteConfigClicked()
{
    ...
    QMessageBox::StandardButton reply = QMessageBox::question(
        this, "确认删除", QString("确定要删除配置 \"%1\" 吗？").arg(configName), QMessageBox::Yes | QMessageBox::No);
...
void AIConfigDialog::updateButtonStates()
{
    ...
    bool hasCredentials =
        !m_baseUrlLineEdit->text().trimmed().isEmpty() && !m_apiKeyLineEdit->text().trimmed().isEmpty();
